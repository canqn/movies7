...
const buttonClass = computed(() => {
  return cva(
    'inline-flex items-center justify-center px-3 py-2 min-w-[40px] bg-first-color-alt-c rounded-lg text-xs ',
    {
      variants: {
        intent: {
          primary: 'text-text-color-light-c hover:bg-yellow-color-c hover:text-white',
          secondary: 'bg-black/5 hover:bg-black/10 text-gray-700',
          danger: 'bg-red-600 text-white hover:bg-red-500',
          text: 'text-gray-700 hover:bg-black/5',
          active: 'bg-yellow-color-c text-white'
        },
        disabled: {
...
<template>
  <component :is="props.as" :disabled="props.disabled" :class="buttonClass">
    ...
    <component :is="props.iconRight" v-if="!props.loading" :class="['ml-2 h-5 w-5', props.loading && 'invisible']" />
  </component>
</template>
